    ...
    override suspend fun execute(input: LogoutUseCase.Input): LogoutUseCase.Output {
        return withContext(Dispatchers.IO) {
            when (val result = authRepository.logout()) {
                is AuthResponse.Success -> LogoutUseCase.Output.Success
                is AuthResponse.Error ->LogoutUseCase.Output.Failure(result.message)
                AuthResponse.Logout -> LogoutUseCase.Output.Success
            }
        }
    }
}
